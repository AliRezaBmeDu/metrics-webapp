...
export const getStocks = createAsyncThunk('stocks/getStock', async () => {
  try {
    const response = 
    await fetch(`https://financialmodelingprep.com/api/v3/stock-screener?limit=20&exchange=nyse,nasdaq,amex&priceMoreThan=100&marketCapLowerThan=5357210641370&isActivelyTrading=true&apikey=${apiKey}`);
...
const stocksSlice = createSlice({
    ...
    extraReducers: {
      ...
      [getStockDetail.fulfilled]: (state, action) => {
          ...
          const { companyName, symbol, description, mktCap, image, price } = stock;
          const details = {
              name: companyName,
              symbol,
              description,
